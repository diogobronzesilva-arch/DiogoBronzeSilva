...
## 13. Quarta passagem (auditoria responsiva, 20 de Agosto de 2026)

Auditoria de layout feita às páginas publicadas em larguras de 320, 375, 390, 430, 768, 1024 e 1440 px, com verificação de overflow horizontal e alinhamento dos componentes principais.

- **`Selected Notes` na home corrigido em mobile**: `.notes-preview` usava o shorthand `padding: ... 0 0`, que anulava sem querer o `padding-left` e `padding-right` herdados da classe `.shell`. O bloco ficava colado às margens do ecrã. Passou a definir apenas `padding-top`, preservando o gutter global de 1,5rem.
- **Cabeçalhos de página corrigidos**: `.pagehead` tinha o mesmo problema de shorthand e fazia os títulos e introduções de Notes, Contact e 404 perderem o gutter horizontal. Passou a definir apenas `padding-top` e `padding-bottom`.
- **Verificação responsiva**: home, índice de Notes, Contact, 404 e os cinco artigos foram verificados sem overflow horizontal nas larguras testadas. Hero, rooms, artigos, newsletter, canais de contacto, masthead e colofão mantêm o gutter correctamente.
- **Navegação mobile**: em ecrãs estreitos a navegação pode ocupar duas linhas, mas mantém-se dentro do viewport e sem corte. Quando a página Work for adicionada ao menu, convém rever especificamente esta zona porque passará a existir mais um item.
